...
check('validation optimization liquidity hunter and global autopilot status preserved', evidence.includes('onRunValidation') && evidence.includes('onRunOptimization') && evidence.includes('onRunLiquidityHunter') && evidence.includes('Global server-owned Autopilot status') && !evidence.includes('SmartAutopilotMiniToggle'));
...
check('compare dialog explains evidence limits', compare.includes('Select up to four registered strategies') && compare.includes('holdout measurements appear only when each model has bound evidence from the same dataset context') && compare.includes('Not comparable'));
