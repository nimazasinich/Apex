...
function downloadCsv(records: WorkspaceActivity[]) {
  ...
  const lines = records.map((record) => [
    record.timestamp == null ? '' : new Date(record.timestamp).toISOString(), record.type, record.title, record.subtitle, record.symbol,
...
function activityTime(timestampValue: number | null): string {
  return timestampValue == null ? '—' : new Date(timestampValue).toLocaleString();
}
