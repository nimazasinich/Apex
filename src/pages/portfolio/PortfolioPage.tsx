...
export function PortfolioPage(props: AccountWorkspaceProps) {
  ...
  const filteredCumulative = useMemo(() => {
    ...
    return cumulative.filter((point) => point.timestamp >= cutoff);
  ...
  const chartLabels = filteredCumulative.length >= 2 ? Array.from({ length: 7 }, (_, index) => {
    const pointIndex = Math.round(index / 6 * (filteredCumulative.length - 1));
    const timestamp = filteredCumulative[pointIndex]?.timestamp;
    if (!Number.isFinite(timestamp)) return '—';
    return new Date(Number(timestamp)).toLocaleString(undefined, performanceWindow === '1d' ? { hour: '2-digit', minute: '2-digit' } : { month: 'short', day: 'numeric' });
  }) : [];

  const allocations = positions
    .filter((position) => position.asset != null && position.valueUsd != null && Number.isFinite(position.valueUsd) && position.valueUsd > 0)
    .slice(0, 6)
    .map((position, index) => ({ ...position, color: ALLOCATION_COLORS[index] }));
  const totalAllocation = allocations.reduce((sum, position) => sum + Number(position.valueUsd), 0);
  ...
  const allocationStops = allocations.map((position) => {
    ...
    allocationCursor += totalAllocation ? positionValue / totalAllocation * 100 : 0;
  ...
  const connectionAvailable = props.connection.status === 'demo' || props.connection.status === 'connected';
    ...
    <div className="v20-reference-page v20-portfolio-page">
      <main className="v20-portfolio-main">
        <div className="v20-metrics six v20-portfolio-metrics">
          <SoftMetric label="Total Equity" value={fmtMoney(equity, currency)} detail={props.connection.mode === 'demo' ? 'Virtual wallet' : 'Exchange reported'} icon={CircleDollarSign} accent="green" chart={performanceValues.length >= 2 ? performanceValues : undefined} sparkBars={false} />
          <SoftMetric label="Available Balance" value={fmtMoney(available, currency)} detail="Available for execution" icon={WalletCards} accent="blue" chart={undefined} sparkBars={false} />
          <SoftMetric label="Unrealized P&L" value={fmtMoney(unrealized, currency)} detail="Open positions only" icon={BarChart3} accent="violet" chart={cumulative.length >= 2 ? cumulative.map((point) => point.value) : undefined} valueTone={tone(unrealized)} sparkBars={false} />
          <SoftMetric label="Position Margin" value={fmtMoney(positionMargin, currency)} detail="Reported position margin" icon={Percent} accent="amber" chart={undefined} sparkBars={false} />
          <SoftMetric label="Order Margin" value={fmtMoney(orderMargin, currency)} detail="Reserved by open orders" icon={Landmark} accent="blue" chart={undefined} sparkBars={false} />
          <SoftMetric label="Frozen Funds" value={fmtMoney(frozen, currency)} detail="Unavailable funds" icon={LockKeyhole} accent="red" chart={undefined} sparkBars={false} />
        </div>
            ...
            {chart ? (
              <div className="v20-portfolio-chart">
                <svg viewBox={`0 0 ${chart.width} ${chart.height}`} preserveAspectRatio="none" role="img" aria-label="Portfolio performance curve">
                  ...
                  {chartLabels.map((label, index) => <text key={`${label}-${index}`} className="axis" x={chart.left + index / 6 * (chart.width - chart.left - chart.right)} y={chart.height - 5} textAnchor={index === 0 ? 'start' : index === 6 ? 'end' : 'middle'}>{label}</text>)}
                </svg>
              </div>
          ...
          <section className="v20-portfolio-card v20-portfolio-allocation">
            ...
            {allocations.length ? (
              <div className="v20-portfolio-allocation-body">
                ...
                <div className="v20-portfolio-allocation-list">{allocations.map((position) => { const positionValue = Number(position.valueUsd); return <div key={position.id}><i style={{ background: position.color }} /><span>{position.asset}</span><strong>{fmtMoney(positionValue, currency)}</strong><em>{totalAllocation ? `${(positionValue / totalAllocation * 100).toFixed(1)}%` : '—'}</em></div>; })}</div>
              </div>
            ) : (
        ...
        <div className="v20-portfolio-bottom-grid">
          <section className="v20-portfolio-card v20-portfolio-holdings">
            ...
            {positions.length ? (
              <div className="v20-portfolio-table-wrap"><table><thead><tr><th>Market</th><th>Side</th><th>Size</th><th>Entry</th><th>Mark</th><th>Unrealized P&L</th><th>Leverage</th></tr></thead><tbody>{positions.map((position) => { const symbol = position.symbol ?? '—'; return <tr key={position.id}><td><CoinIcon symbol={position.symbol ?? ''} size={22} /><span><strong>{symbol}</strong><small>Perpetual</small></span></td><td><span className={`v20-pill ${position.side === 'LONG' ? 'success' : position.side === 'SHORT' ? 'danger' : 'muted'}`}>{position.side}</span></td><td>{position.size == null ? '—' : position.size.toLocaleString()}</td><td>{fmtPrice(position.entryPrice)}</td><td>{fmtPrice(position.markPrice)}</td><td className={tone(position.unrealizedPnlUsd)}>{fmtMoney(position.unrealizedPnlUsd, currency)}</td><td>{position.leverage == null ? '—' : `${position.leverage.toFixed(position.leverage % 1 ? 2 : 0)}x`}</td></tr>; })}</tbody></table></div>
          ...
          <section className="v20-portfolio-card v20-portfolio-health">
            ...
            <div className="v20-health-bars">
              ...
              <div><span>Connection Status</span><i><b style={{ width: connectionAvailable ? '100%' : '0%' }} /></i><em>{connectionAvailable ? 'CONNECTED' : 'DISCONNECTED'}</em></div>
            </div>
