...
// Synthetic Trading Lab data is a development-only visual QA fixture.
// Production builds hard-disable it even when the VITE flag is present.
const TRADING_LAB_PREVIEW_ENABLED =
  import.meta.env.DEV && import.meta.env.VITE_APEX_TRADING_LAB_PREVIEW === 'true';

function buildTradingLabCandles(startPrice: number, count = 84, intervalMs = 15 * 60 * 1000): Candle[] {
...
function buildTradingLabDepth(lastPrice: number, side: 'bid' | 'ask', count = 12) {
  let cumulative = 0;
  return Array.from({ length: count }, (_, index) => {
...
function buildTradingLabPreview(settings: TerminalSettings) {
  const now = Date.now();
  ...
  const tickers: SymbolTicker[] = [
    ...
    },
    {
      ...
      openInterest: 824_000_000, dataState: 'live', timestamp: now,
    ...
    },
    {
      ...
      openInterest: 263_000_000, dataState: 'live', timestamp: now,
  ...
  const chartOrderBookLevels: OrderBook = { bids, asks, dataSource: 'live' };
  ...
  const levels: DerivedLevels = {
    ...
    confidenceScore: 83,
    evidenceList: [
      { label: 'Trend', tag: 'supports', detail: '15m and 1h structure are aligned in the laboratory scenario.' },
  ...
  const baseSizing = {
    ...
    summaryText: 'Laboratory sizing: 3 contracts at controlled demo risk.',
  };
  const tradePlanLong: TradePlan = {
    version: 'trade_plan_v1', id: 'LAB-BTC-LONG-01', symbol: selectedTicker.symbol, direction: 'LONG',
    decisionRef: { score: longScore.score, readinessTier: longScore.readinessTier, engineVersion: 'lab_preview_v1', createdAt: now },
  ...
  const tradePlanShort: TradePlan = {
    ...tradePlanLong,
    id: 'LAB-BTC-SHORT-01', direction: 'SHORT',
    decisionRef: { score: shortScore.score, readinessTier: shortScore.readinessTier, engineVersion: 'lab_preview_v1', createdAt: now },
  ...
  const connection: ConnectionState = {
    ...
    profile: { id: 'lab-preview', name: 'APEX Lab Preview', accountType: 'DEMO' },
  ...
  const chartFeed: ChartFeedStatus = { loading: false, dataState: 'live', source: 'LAB_PREVIEW', stale: false, ageMs: 640, error: null };
...
function TradingViewCore({ settings, tickers = [], selectedTicker, onSelectSymbol, levels, longScore, shortScore, tradePlanLong, tradePlanShort, chartCandles, chartOrderBook, chartOrderBookLevels, chartInterval, chartFeed, onRetryChart, onChartIntervalChange, ...accountProps }: TradingViewProps) {
  ...
  const [systemContext, setSystemContext] = useState(() => readWorkspaceContext() ?? (TRADING_LAB_PREVIEW_ENABLED ? { source: 'trading' as const, updatedAt: Date.now(), strategyId: 'momentum-breakout-lab', strategyName: 'Momentum Breakout', symbol: 'BTC-USDT', direction: 'LONG' as const, interval: '1h' as const } : null));
  ...
  const systemLinkContext: SystemLinkContext = {
    strategyName: systemContext?.strategyName || (TRADING_LAB_PREVIEW_ENABLED ? 'Momentum Breakout' : undefined),
    ...
    lastBacktest: matchesBacktestEvidence(systemContext, activeEvidenceIdentity)
      ...
      : TRADING_LAB_PREVIEW_ENABLED
        ? { netReturnPct: 12.45, maxDrawdownPct: 5.90, trades: 162 }
        : null,
  };
...
export function TradingView(props: TradingViewProps) {
  const lab = useMemo(() => TRADING_LAB_PREVIEW_ENABLED ? buildTradingLabPreview(props.settings) : null, [props.settings]);
  if (!lab) return <TradingViewCore {...props} />;
  return <TradingViewCore
    {...props}
    ...
    error={null}
    tickers={lab.tickers}
    selectedTicker={lab.selectedTicker}
    levels={lab.levels}
    longScore={lab.longScore}
    shortScore={lab.shortScore}
    tradePlanLong={lab.tradePlanLong}
    tradePlanShort={lab.tradePlanShort}
    chartCandles={lab.chartCandles}
    chartOrderBook={lab.chartOrderBook}
    chartOrderBookLevels={lab.chartOrderBookLevels}
    chartFeed={lab.chartFeed}
    chartInterval="15m"
  />;
}
